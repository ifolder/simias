...
public class Conflict
{
	...
	/// </summary>
	public void SaveToExternalConflictBin(bool localChangesWin, string conflictBinPath)
	{
		try
		{
			string ConflictFileFullPath = String.Empty;
			if(localChangesWin)
				ConflictFileFullPath = conflictBinPath+conflictNode.Name+node.MasterIncarnation.ToString()+".server." +"."+collection.ID;
			else
				ConflictFileFullPath = conflictBinPath+node.Name+node.LocalIncarnation.ToString()+".local."+"."+collection.ID;
			
			Log.log.Debug("Save to external ConflictFileFullPath:{0}",ConflictFileFullPath);
			Log.log.Debug("Save to external UpdateConflictPath:{0}",UpdateConflictPath);
			Log.log.Debug("Save to external NonconflictedPath:{0}",NonconflictedPath);
			
			if(localChangesWin)			
				File.Copy(UpdateConflictPath, ConflictFileFullPath, true);			
			else			
				File.Copy(NonconflictedPath, ConflictFileFullPath, true);			
			
			//ConflictFileFullPath +=".properties.txt";
			//File.Create(ConflictFileFullPath);
			//File.Open(node.GetFullPath(collection), FileMode.Open, FileAccess.Read, FileShare.Read);
		}
		catch (Exception ex)
		{
			throw ex;
    ...
    /// resolve update conflict and commit 
    /// </summary>
    public void Resolve(bool localChangesWin, string conflictBinPath)
    {
        if (conflictNode == null)
            return;

        if (conflictBinPath != String.Empty)
            SaveToExternalConflictBin(localChangesWin, conflictBinPath);

        Resolve(localChangesWin);
    }
    ...
    /// does not resolve file name conflicts.
    /// </summary>
    public static void Resolve(Collection collection, bool localChangesWin, string conflictBinPath)
    {
        foreach (ShallowNode sn in collection.GetCollisions())
        {
            Conflict cf = new Conflict(collection, new Node(collection, sn));
            cf.Resolve(localChangesWin, conflictBinPath);
        }
    }
