...
*        < SyncInterval Policy Implementation >
*
*
*******************************************************************************/
...
namespace Simias.Policy
{
	...
	public class SyncInterval
	{
		...
		/// </summary>
		static public readonly string IntervalTag = "Interval";
		...
		static public void Create( Member member, int interval )
		{
			...
			if ( ( interval == InfiniteSyncInterval ) || ( interval > 0 ) )
			{
				pm.CommitPolicy( SyncIntervalPolicyID, interval, member);
			}
			else
			{
				// Setting the interval to zero is the same as deleting the policy.
				pm.DeletePolicy( SyncIntervalPolicyID, member );
			}
		}
		...
		static public void Create( int interval )
		{
			...
			if ( ( interval == InfiniteSyncInterval ) || ( interval > 0 ) )
			{
                ...
                //complete the job in current queue (sync) by syncing them immediately

                //this wud start the new policy immediately
                /*Store store = Store.GetStore();
                if (store != null)
                {
                    ICSList domainList = store.GetDomainList();

               foreach (ShallowNode dom in domainList)
                    {
                        Sync.SyncClient.RescheduleAllColSync(dom.ID);
                    }
                }*/
			}
			else if ( policy != null )
