...
*        < Disk Quota Policy Implementation >
*
*
*******************************************************************************/
...
namespace Simias.Policy
{
	...
	public class DiskSpaceQuota
	{
        ...
        /// </summary>
        static public string UsedDiskSpaceOnServer = "Used Disk Space On Server";
		...
		{
			PolicyManager pm = new PolicyManager();
			// In New implementation, all policies are part of member object so get full member object first.
			...
			if(member == null)
			{
				// member should not be null , but if it is , then we use old code.
...
//			this.usedDiskSpace = GetUsedDiskSpace( member );
		...
		static public void Create( Member member, long limit )
		{
			...
			if ( limit >= 0)
			{
				...
				pm.CommitPolicy(DiskSpaceQuotaPolicyID, ftList, member);
			}
			else
			{
				pm.DeletePolicy( DiskSpaceQuotaPolicyID, member );
			}
		}
		...
		static private Rule GetUserRule( Policy policy )
		{
			...
			if ( !e.MoveNext() )
			{
				throw new SimiasException( "There is no policy rule on quota." );
			}
