...
namespace Simias.Storage
{
	...
	internal class ChangeLogWriter : IDisposable
	{
		...
		/// </summary>
		public static uint DefaultMaxPersistedRecords
		{
			get { return defaultMaxPersistedRecords; }
		}
	...
	public class ChangeLogReader
	{
		...
		/// <summary>
		/// enum to represent different status while reading changelog file.
		/// </summary>
		private enum ReadChangeLogStatus
		{
			// No record found at this file position
			NoRecordFound = 1,
			// Record was found at this position and there is no rollover in changelog file
			RecordFound = 2,
			// Record was found at this position, but this position is at bottom and there is a Rollover in changelog
			AtEOFAndRollover = 4,
			// Record was found at this position, but this position is not at bottom and there is a Rollover in changelog
			NoEOFAndRollover = 8
		}
		#endregion
		...
		/// <returns>1 if next offset was not found.</returns>
		/// <returns>2 if the next offset was found and no rollover.</returns>
		/// <returns>4 (rollover case) if the next offset was found at EOF, and top of file has new records too .</returns>
		/// <returns>8 (rollover case) if the next offset was found, not at EOF, but after this offset older records exist.</returns>
		private int GetReadPosition( FileStream fs, EventContext cookie )
		{
			int foundOffset = (int)ReadChangeLogStatus.NoRecordFound;
			int bytesRead = 0;
			byte[] buffer = new byte[ ChangeLogRecord.RecordSize ];
				...
				try
				{
					...
					if ( bytesRead > 0 )
					{
						...
						if ( ( record.RecordID == cookie.RecordID ) && ( ( record.Epoch == cookie.TimeStamp ) || ( cookie.TimeStamp == DateTime.MinValue ) ) )
						{
							...
							foundOffset = (int) ReadChangeLogStatus.RecordFound;
							long CurrPos = fs.Position;
							bool eofreached = false;
							byte[] tmpbuffer = new byte[ ChangeLogRecord.RecordSize ];
							
							if(fs.Position >= ((ChangeLogWriter.DefaultMaxPersistedRecords * ChangeLogRecord.RecordSize)+LogFileHeader.RecordSize))
							{
								fs.Position = LogFileHeader.RecordSize;
								eofreached = true;
							}
							bytesRead = fs.Read( tmpbuffer, 0, tmpbuffer.Length );
							if(bytesRead > 0 )
							{
								ChangeLogRecord tmprecord = new ChangeLogRecord( tmpbuffer );
								foundOffset = ( eofreached &&  tmprecord.RecordID > record.RecordID) ? (int) ReadChangeLogStatus.AtEOFAndRollover : ( tmprecord.RecordID < record.RecordID ? (int) ReadChangeLogStatus.NoEOFAndRollover : (int) ReadChangeLogStatus.RecordFound); 
							}
							fs.Position = (foundOffset == (int) ReadChangeLogStatus.AtEOFAndRollover) ? LogFileHeader.RecordSize : CurrPos;
						}
					}
							...
							while ( bytesRead > 0 )
							{
								...
								while ( ( index + ChangeLogRecord.RecordSize ) <= bytesRead )
								{
									...
									if ( record1.RecordID > record2.RecordID )
									{
										...
										bytesRead = 0;
										log.Debug("GetEventContext: Record id has rolled over.");
										break;
									}
									else
									{
		...
		public bool GetEvents( EventContext cookie, out ArrayList changeList )
		{
			...
			int OffSetState = (int) ReadChangeLogStatus.NoRecordFound;
			...
			try
			{
				try
				{
					...
					try
					{
						...
						OffSetState = GetReadPosition( fs, cookie ); 
						if(OffSetState == (int) ReadChangeLogStatus.NoRecordFound)
						{
						...
						if(OffSetState != (int) ReadChangeLogStatus.NoEOFAndRollover)
							bytesRead = fs.Read( buffer, 0, buffer.Length );
			...
			ChangeLogRecord newRecord=null, oldRecord = null; 
			// Make sure that something was read.
			if(OffSetState != (int) ReadChangeLogStatus.NoEOFAndRollover)
			{
				// this is the case when recordids are arranged in normal case
				// OffSetState = 8 means, higher recordid is followed by lower recordid at current fs.position, so don't add to changelist
				for ( int i = 0; i < bytesRead; i += ChangeLogRecord.RecordSize )
				{
					newRecord  = new ChangeLogRecord( buffer, i );
					if(oldRecord == null)
					{
						oldRecord = new ChangeLogRecord( buffer, i );
					}
					if(newRecord.RecordID  >= oldRecord.RecordID)
					{
						changeList.Add( newRecord );
					}
					else
					{
						log.Debug("GetEvents : Rollover point found, Lower record ids ahead, avoid adding to changelog and break");
						break;
					}
					lastRecordOffset += ChangeLogRecord.RecordSize;
					oldRecord = new ChangeLogRecord( buffer, i );
				}
			}
