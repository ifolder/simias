...
namespace Simias.Storage
{
	...
	public class Property
	{
		...
		internal const uint ServerOnly  = 0x00080000;
		...
		/// <summary>
		/// Gets and sets the transient status of a property.  If this flag is false, this property will
		/// synchronize to other slaves and client also.  If it is true, it is a property local to server only
		/// flag is set to false by default.
		/// </summary>
		public bool ServerOnlyProperty 
		{
			get { return ( PropertyFlags & ServerOnly ) == ServerOnly ? true : false; }

			set
			{
				if ( value )
				{
					PropertyFlags |= ServerOnly;
				}
				else
				{
					PropertyFlags &= ~ServerOnly;
