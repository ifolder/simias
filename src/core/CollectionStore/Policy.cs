...
namespace Simias.Policy
{
	...
	public class Policy : Node
	{
		...
		public ICSList Rules
		{
			...
			{ 
				...
				{
					// since system policies will be in the form of "RuleList" , we need to add that also 
					MultiValuedList mvl2 = policy.properties.GetProperties( RuleList );
					if(mvl2.Count > 0)
					{
						foreach( Property p in mvl2 )
						{
							ruleList.Add( new Rule( p.Value ) );
						}
					}
					// Now add, if its member policy object , then it will have policyid based storage
					Property PolicyProp = policy.Properties.GetSingleProperty(PropertyTags.PolicyID);
					string policyID = null;
					if(PolicyProp != null )
					{
						policyID = PolicyProp.Value as string;
						// Get the rule list.
						MultiValuedList mvl = policy.properties.GetProperties( policyID );
						if(mvl.Count > 0)
						{
		...
		/// <param name="PolicyID">this is the policyID for a policy which has come from a member policy , For user and system policy ,its null</param>
		/// <returns>An array of exception rules from the policy.</returns>
		private Rule[] GetExceptionList( Policy policy, string PolicyID )
		{
			ArrayList exceptionList = new ArrayList();

			// Get the rule list.
			MultiValuedList mvl;
			mvl = policy.properties.GetProperties(RuleList );
                        if(mvl == null || mvl.Count == 0)
                        {
                                if(PolicyID != null)
                                        mvl = policy.properties.GetProperties(PolicyID );
                        }
			...
			foreach( Property p in mvl )
			{
                                ...
                                if(p.Type == Syntax.String)
                                       rule = new Rule( p.Value as string );
                                else
                                       rule = new Rule( p.Value );
		...
		private Rule[] GetExclusionList( Policy policy, string PolicyID )
		{
		...
		/// <param name="PolicyID">If member Policy object has called it,then pass the specific policyid , for other policy objects , pass null</param>
		/// <returns>True if the policy allows the operation, otherwise false is returned.</returns>
		public Rule.Result Apply( object input, string PolicyID )
		{
			...
			foreach ( Policy policy in policyArray )
			{
				...
				if ( ( p == null ) || ( new PolicyTime( p.Value as string ).Apply() == Rule.Result.Allow ) )
				{
					...
					foreach ( Rule exception in GetExceptionList( policy, PolicyID ) )
					{
						// Apply the exception rule to see if it passes.
						if ( exception.Apply( input ) == Rule.Result.Allow )
						{
							ResultValue = Rule.Result.Allow;
						}
					}
					// Check the exclude list to see if it passes.
					foreach ( Rule exclusion in GetExclusionList( policy, PolicyID ) )
					{
	...
	public class PolicyManager 
	{
		...
		public void CommitPolicy( string policyID, ICSList ruleList, Member member)
		{
			...
			if(policyID != null)
			{
				member.Properties.DeleteNodeProperties(policyID);
				foreach ( Rule rule in ruleList )
				{
					Property p = new Property( policyID, rule.ToString() );
					p.ServerOnlyProperty = true;
					member.Properties.AddNodeProperty( p );
				}
		...
		public void CommitPolicy( string policyID, object value, Member member)
		{
			...
			if(value != null)
			{
				Property p = new Property( policyID, value );
				p.ServerOnlyProperty = true;
		...
		/// </summary>
		/// <param name="policyID">Policy identifier to delete.</param>
		/// <param name="member">member object whom policy is attached to.</param>
		public void DeletePolicy( string policyID, Member member )
		{
			string domainID = member.GetDomainID( store );
			if ( domainID == null )
			{
		...
		public Policy GetAggregatePolicy( string policyID, Member member, bool includeExceptions, bool includeLocalMachinePolicy )
		{
			...
			if ( ( policy == null ) || includeExceptions )
			{
				...
				if( Simias.Service.Manager.LdapServiceEnabled == true )
				{
					...
					string[] GIDs = domain.GetMemberFamilyList(member.UserID);
                        		foreach(string gid in GIDs)
                        		{
                                		...
                                		{
							Member groupMember = domain.GetMemberByID(gid);
							Policy groupMemberPloicy = GetPolicy( policyID, groupMember );
							if( groupMemberPloicy != null )
							{
								if ( policy == null )
								{
									policy = groupMemberPloicy;
								}
								else
								{
									policy.AddAggregatePolicy( groupMemberPloicy );
								}
								break;
							}
				...
				if ( (policy == null || includeExceptions) && domainID != null )
				{
		...
		public Policy GetPolicy( string policyID, Member member )
		{
			...
			if ( domainID != null )
			{
				// new implementation , all policies for a member will be stored/retrieved as part of user object
				MultiValuedList mvl = member.Properties.GetProperties(policyID);
				if(mvl == null || mvl.Count == 0)
				{
					return policy;
				}

				string PolicyDescription = "No Description";
				Policy tempPolicy = new Policy(policyID, PolicyDescription);

				// For a value based policy , Tag will be associated based on policyID

				string PolicyTag = null;
				if(policyID.Equals(Simias.Policy.Sharing.SharingPolicyID))
				{
					PolicyTag = Simias.Policy.Sharing.SharingStatusTag;
				}
				else if(policyID.Equals(Simias.Policy.SyncInterval.SyncIntervalPolicyID))
				{
					PolicyTag = Simias.Policy.SyncInterval.IntervalTag;
				}
				else if(policyID.Equals(Simias.Policy.SecurityState.EncryptionStatePolicyID))
				{
					PolicyTag = Simias.Policy.SecurityState.StatusTag;
				}
				...
				foreach( Property p in mvl )
				{
					if(PolicyTag != null)
					{
						// It means , its a value based policy , so fill the policy with tag and value
						tempPolicy.AddValue( PolicyTag, (int)p.Value );
					}
					else
					{
						// For rule based policies , form the rule and then add it to policy object
						// Based on type different constructors are called. This is used 
						// specially in case of FileType policies.
						Rule rule = null;
						if(p.Type == Syntax.String)
							rule = new Rule( p.Value as string );
						else 
							rule = new Rule( p.Value  );
						tempPolicy.Properties.AddNodeProperty( policyID, rule.ToXml() );
					}
				}	
					policy = tempPolicy;
			}
	...
	public class Rule : ISerializable
	{
		...
		/// </summary>
		/// <param name="rule">Xml document of string format that contains a rule representation.</param>
		public Rule( string rule )
		{
			XmlDocument ruleDocument = new XmlDocument();
			ruleDocument.LoadXml(rule);
			XmlElement element = ruleDocument.DocumentElement;
