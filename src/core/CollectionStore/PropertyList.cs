...
namespace Simias.Storage
{
	...
	public class PropertyList : IEnumerable, ISerializable
	{
		...
		/// </summary>
		public XmlElement PropertyRoot
		{
 		...
 		internal void StripLocalNServerLocalProperties( XmlDocument document, bool LoggedInMember )
 		{
		    ...
		    foreach ( XmlNode currentNode in nodeArray )
		    {
			    ...
			    else
			    {
	 			    ...
	 			    if ( ( flags & Property.ServerOnly ) == Property.ServerOnly )
 			    	    {
					    // It is policy related tag
				    	    if( ! LoggedInMember )
				    	    {
					    	    // logged-in member is syncing other member objects , so strip off this policy property document.
 				    	    	    // Remove this property out of the document.
				    	    	    currentNode.ParentNode.RemoveChild (currentNode);
				    	     }
				    }	
		...
		{
			XmlDocument stringDoc = nodeDocument.Clone() as XmlDocument;
			StripLocalNServerLocalProperties( stringDoc, LoggedInMember );
