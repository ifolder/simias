	...
	public class Authentication : IDomainProvider
	{
		...
		AuthenticateByName( string DomainID, string User, string Password )
		{
			...
			try
			{
				...
				{
					log.Info( "Authenticated User iS : {0}:{1} {2}", status.UserID, status.UserName, status.statusCode );
				}
				else
				{
									...
									if( Domain.IsLoginDisabled( member.UserID ) != true )
									{
										try
										{
											...
											HostNode mNode = member.HomeServer;
                                                                                        log.Debug("id.Auth : localhost ip is :"+hNode.PublicUrl.ToString());
											Http.UserMoved = 0;
											if(mNode != null)
											{
                                                                                        	log.Debug("id.Auth : this persons homeadd ip is :"+mNode.PublicUrl.ToString());
										...
										{
											log.Error( e.Message+"The message should be catch properly" );
