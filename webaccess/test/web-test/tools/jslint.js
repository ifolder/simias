...
return syntax['function'];},155).exps=true;prefix('(',function(){parse(0);advance(')',this);});infix('[',function(left){var e=parse(0);if(e&&e.type==='(string)'){countMember(e.value);}
...
return;}
advance();}}})();jslint.laxLineEnd=false;jslint.plusplus=false;jslint.cap=true;jslint.jscript=false;jslint.debug=false;if(!jslint(WScript.StdIn.ReadAll())){var e=jslint.error;WScript.StdErr.WriteLine('Lint at line '+(e.line+1)+' character '+(e.character+1)+': '+e.reason);WScript.StdErr.WriteLine((e.evidence||'').replace(/^\s*(\S*(\s+\S+)*)\s*$/,"$1"));WScript.Quit(1);}