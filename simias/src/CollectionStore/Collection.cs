...
namespace Simias.Storage
{
	...
	public class Collection : Node, IEnumerable
	{
		...
		/// <summary>
		/// Gets a list of ShallowNode objects that represent Node objects that
		/// did not synchronize.  This list will not contain Nodes that have
		/// the "IgnoreSyncStatus" property.
		/// </summary>
		/// <returns>An ICSList object containing ShallowNode objects
		/// representing Node objects that did not synchronize.</returns>
		public ICSList GetUnsynchronizedNodes()
		{
			ICSList list = new ICSList();
			
			// Add on nodes with the SyncStatus property
			ICSList syncStatusList = Search( PropertyTags.SyncStatusTag, Syntax.String );
			foreach(ShallowNode sn in syncStatusList)
			{
				Node node = GetNodeByID(sn.ID);
				if (node.Properties.GetSingleProperty("IgnoreSyncStatus") == null)
					list.Add(sn);
			}
			
			// Add on collision nodes
			ICSList collisionList = GetCollisions();
		...
		/// <summary>
		/// Gets a list of ShallowNode objects that represent Node objects that
		/// did not synchronize AND that have been marked to be ignored by the
		/// user.
		/// </summary>
		/// <returns>An ICSList object containing ShallowNode objects
		/// representing Node objects that did not synchronize which have been
		/// ignored by the user.</returns>
		public ICSList GetIgnoredUnsynchronizedNodes()
		{
			return Search("IgnoreSyncStatus", Syntax.Boolean);
		}
		...
		/// <summary>
		/// Returns whether the collection has unsynchronized nodes.
		/// </summary>
		/// <returns>True if the collection contains unsynchronized nodes, otherwise false is returned.</returns>
		public bool HasUnsynchronizedNodes()
		{
			ICSEnumerator e = GetUnsynchronizedNodes().GetEnumerator() as ICSEnumerator;
			bool hasUnsynchronizedNodes = e.MoveNext();
			e.Dispose();
			return hasUnsynchronizedNodes;
		}
